...
usage() {
  cat <<'USAGE'
Usage: scan-flac.sh [--install-brew-deps] [--output-dir DIR] FILE_OR_DIR...
USAGE
}
...
if (( install_deps )); then
  ...
  brew install flac ffmpeg media-info exiftool clamav
fi

for cmd in flac metaflac ffprobe mediainfo exiftool; do
...
if [[ -z "$output_root" ]]; then
  output_root="./flac-scan-$(date +%Y%m%d-%H%M%S)"
fi
mkdir -p "$output_root"
...
collect_files() {
  ...
  for p in "$@"; do
    if [[ -d "$p" ]]; then
      find "$p" -type f \( -iname '*.flac' -o -iname '*.fla' \) -print
    elif [[ -f "$p" ]]; then
      printf '%s\n' "$p"
    else
...
if [[ ${#files[@]} -eq 0 ]]; then
  err "no FLAC files found"
  exit 1
fi

summary_tsv="$output_root/summary.tsv"
printf 'file\tclamav\tffprobe\tmediainfo\texiftool\tflac_test\tmetaflac\n' > "$summary_tsv"
...
for f in "${files[@]}"; do
  ...
  if flac -t --totally-silent -- "$f" >"$stem/flac_test.stdout" 2>"$stem/flac_test.stderr"; then
    flac_status="OK"
  else
    flac_status="ERR"
  fi

  if metaflac --show-md5sum --show-sample-rate --show-channels --show-bps --show-total-samples -- "$f" >"$stem/metaflac.txt" 2>"$stem/metaflac.stderr"; then
    metaflac_status="OK"
  else
    metaflac_status="ERR"
  fi

  printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\n' "$f" "$clam_status" "$ffprobe_status" "$mediainfo_status" "$exiftool_status" "$flac_status" "$metaflac_status" >> "$summary_tsv"
done
